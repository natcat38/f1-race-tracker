...
def assemble_header(track_points, corners, label, max_rev, radio_clips, lap_trace,
                     total_laps, stints, pit_stops, pedal_traces, sector_dominance):
    """Assemble the clip header dict from already-computed pieces (see the
    module docstring's CONTRACT). Pure dict construction, no I/O.

    # ponytail: extracted verbatim from main()'s emit loop so ingest/test_record.py
    can exercise the recorder's header-wiring (pit/ghost/heatmap/corner data landing
    in the right header fields) without needing a full FastF1 session -- record.py
    is otherwise a top-level script with no importable seam at all.
    """
    return {
        "track": track_points,
        "corners": corners,
...
def assemble_frame(rev, time_ms, cars, messages=None, weather=None):
    """Assemble one frame dict from already-decided pieces (see the module
    docstring's CONTRACT). Pure dict construction, no I/O.

    `cars` must already be reconciled (resample.reconcile_positions) into the
    unique, contiguous 1..N running order the contract requires -- this function
    does not itself enforce that invariant, it only wires the pieces together.

    # ponytail: extracted for the same reason as assemble_header above.
    """
...
def main():
    ...
    with open(OUTPUT_PATH, 'w', encoding='utf-8') as f:
        ...
        header = assemble_header(
            track_points, corners, GP_LABEL, max_rev, radio_clips, lap_traces,
            TOTAL_LAPS, stints, pit_stops, pedal_traces, sector_dominance,
        )
        ...
        for i, t_s in enumerate(t_grid_s):
            ...
            wx = _weather_at(t_s)
            include_weather = wx is not None and (i == 0 or wx != _last_weather)
            if include_weather:
                _last_weather = wx
            frame = assemble_frame(rev, time_ms, cars, msgs_by_idx.get(i), wx if include_weather else None)
