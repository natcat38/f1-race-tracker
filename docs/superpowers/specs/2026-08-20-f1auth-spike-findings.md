...
3. **`XDG_DATA_HOME` mounted read-only breaks fastf1 import.** At import time `f1auth` runs `platformdirs.user_data_dir("fastf1", ensure_exists=True)` and `AUTH_DATA_FILE.touch()`. On a read-only bind mount both raise. So the container must point `XDG_DATA_HOME` at a **writable** dir and copy the mounted token into it before importing fastf1. `./secrets` stays read-only; `f1tv_auth.stage_token()` does the copy.
