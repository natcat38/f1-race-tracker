...
- **Pit-wall timing tower** — beside the map the board shows a live timing tower with gaps/intervals, last lap, tyres, and sector times for every car; click any driver to open a per-car telemetry panel (speed, gear, throttle, brake, DRS, and a lap-time trend sparkline) sourced from the same 10 Hz frame.
- **Sector and lap-pace signal** — session-best and personal-best sector shading, plus a magnitude chip showing exactly how far off personal best a sector is, so a driver "losing time" is visible, not just flagged.
...
- **Race control feed** — a rolling log of real session messages (flags, safety car, investigations), baked from the session's actual race-control feed and replayed in sync alongside the timing tower.
